...
public class ProductoDAO implements IProductoDAO {
    ...
    public List<Producto> obtenerProductos() throws ModuloProductoPersistenciaException {
        ...
        EntityManager em = Conexion.crearConexion();
        try {
            return em.createQuery("SELECT p FROM Produicto p", Producto.class).getResultList();
        } catch (Exception e) {
            throw new ModuloProductoPersistenciaException("Error al obtener todos los productos: " + e.getMessage());
        } finally {
            em.close();
        }
    }
